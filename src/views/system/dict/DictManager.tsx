    ...
    <Row gutter={16} wrap={false} align="top">
        ...
        >
          ...
          <Table
            ...
            onRow={(record) => ({
              ...
              style: {
                cursor: 'pointer',
                background: current?.dictId === record.dictId ? 'rgba(37,99,235,0.08)' : undefined
              }
            })}
            columns={[
              {
      ...
      <Modal open={typeDlg.open} title={typeDlg.row ? '修改字典类型' : '新增字典类型'} onCancel={() => setTypeDlg({ open: false })} onOk={saveType} destroyOnClose maskClosable={false}>
        <Form form={typeForm} layout="vertical" style={{ marginTop: 8 }}>
          ...
          <Form.Item name="status" label="状态"><Radio.Group optionType="button" buttonStyle="solid">{STATUS_OPTIONS.map((o) => <Radio key={o.value} value={o.value}>{o.label}</Radio>)}</Radio.Group></Form.Item>
      ...
      <Modal open={dataDlg.open} title={dataDlg.row ? '修改字典数据' : '新增字典数据'} onCancel={() => setDataDlg({ open: false })} onOk={saveData} destroyOnClose maskClosable={false}>
        <Form form={dataForm} layout="vertical" style={{ marginTop: 8 }}>
          <Row gutter={16}>
            ...
            <Col span={12}><Form.Item name="status" label="状态"><Radio.Group optionType="button" buttonStyle="solid">{STATUS_OPTIONS.map((o) => <Radio key={o.value} value={o.value}>{o.label}</Radio>)}</Radio.Group></Form.Item></Col>
