...
export default function AgentPage() {
  ...
  const getFuncMenu = (funcCode: string): MenuProps['items'] => [
    {
      key: 'open',
      icon: <ExportOutlined />,
      label: '前往「AI模型功能配置」'
  ...
  const onFuncMenuClick = (funcCode: string): MenuProps['onClick'] => ({ key, domEvent }) => {
    ...
    if (key === 'open') {
      ...
      window.open('/ai-model/funcconfig', '_blank', 'noopener,noreferrer');
    } else if (key === 'filter') {
  ...
  /** 默认模型 code 的下拉菜单 */
  const getModelMenu = (modelCode: string): MenuProps['items'] => [
    {
      key: 'open',
      icon: <ExportOutlined />,
      label: '前往「AI模型配置」'
  ...
  const onModelMenuClick = (modelCode: string): MenuProps['onClick'] => ({ key, domEvent }) => {
    domEvent?.stopPropagation?.();
    if (key === 'open') {
      jumpInApp('/ai-model/aidmodel');
    } else if (key === 'open-blank') {
      window.open('/ai-model/aidmodel', '_blank', 'noopener,noreferrer');
    } else if (key === 'copy') {
    ...
    <div className="crud-page">
      ...
      <Card className="page-card" bordered={false}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18, gap: 16, flexWrap: 'wrap' }}>
          <div>
            ...
            <div style={{ color: '#94a3b8', fontSize: 13, marginTop: 4 }}>
              管理业务智能体（system prompt + 默认模型 + 业务分类联动），后续将支持远程模板拉取与版本回退
            </div>
          </div>
          <Space wrap>
        ...
        <Row gutter={[14, 14]} style={{ marginBottom: 18 }}>
          <Col xs={12} sm={6}>
            <Card size="small" bordered={false} style={{ borderRadius: 12, background: 'linear-gradient(135deg, #eef2ff 0%, #ffffff 100%)', border: '1px solid rgba(99, 102, 241, 0.15)' }}>
              <Statistic title={<span style={{ color: '#6366f1' }}><ThunderboltOutlined /> 智能体总数</span>} value={stats.total} valueStyle={{ color: '#4338ca', fontWeight: 700 }} />
            </Card>
          </Col>
          <Col xs={12} sm={6}>
            <Card size="small" bordered={false} style={{ borderRadius: 12, background: 'linear-gradient(135deg, #ecfdf5 0%, #ffffff 100%)', border: '1px solid rgba(16, 185, 129, 0.15)' }}>
              <Statistic title={<span style={{ color: '#10b981' }}><CheckCircleOutlined /> 已启用（当前页）</span>} value={stats.enabled} valueStyle={{ color: '#059669', fontWeight: 700 }} />
            </Card>
          </Col>
          <Col xs={12} sm={6}>
            <Card size="small" bordered={false} style={{ borderRadius: 12, background: 'linear-gradient(135deg, #f8fafc 0%, #ffffff 100%)', border: '1px solid rgba(148, 163, 184, 0.18)' }}>
              <Statistic title={<span style={{ color: '#64748b' }}><PauseCircleOutlined /> 已停用（当前页）</span>} value={stats.disabled} valueStyle={{ color: '#475569', fontWeight: 700 }} />
            </Card>
          </Col>
          <Col xs={12} sm={6}>
            <Card size="small" bordered={false} style={{ borderRadius: 12, background: 'linear-gradient(135deg, #fffbeb 0%, #ffffff 100%)', border: '1px solid rgba(245, 158, 11, 0.15)' }}>
              <Statistic title={<span style={{ color: '#f59e0b' }}><FileTextOutlined /> 未配默认模型</span>} value={stats.noModel} valueStyle={{ color: '#d97706', fontWeight: 700 }} />
            </Card>
          </Col>
        </Row>
