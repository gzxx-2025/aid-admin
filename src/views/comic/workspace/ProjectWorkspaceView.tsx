...
import { Avatar, Button, Card, Menu, Space, Tag } from 'antd';
import {
...
export default function ProjectWorkspaceView({ project, onBack }: Props) {
  ...
  return (
    <div>
      {/* 项目头部 */}
      <Card bordered={false} className="page-card" style={{ marginBottom: 16 }} styles={{ body: { padding: 18 } }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
          <Button icon={<ArrowLeftOutlined />} onClick={onBack}>返回项目列表</Button>
          {project?.coverUrl
            ? <Avatar shape="square" size={56} src={project.coverUrl} />
            : <Avatar shape="square" size={56} icon={<VideoCameraOutlined />} style={{ background: '#6366f1' }} />}
          <div style={{ flex: 1, minWidth: 200 }}>
            <Space wrap>
              <span style={{ fontSize: 18, fontWeight: 700 }}>{project?.projectName || '未命名项目'}</span>
            ...
            <div style={{ color: '#94a3b8', fontSize: 13, marginTop: 4 }}>
              项目ID：{projectId} · 用户ID：{userId} · 当前步骤：{getLabelByValue(CURRENT_STEP_WITH_MOVIE_OPTIONS, project?.currentStep)}
            </div>
          </div>
        </div>
      </Card>
      ...
      <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
        <Card bordered={false} className="page-card" style={{ width: 200, flexShrink: 0 }} styles={{ body: { padding: 8 } }}>
          <Menu
            mode="inline"
            selectedKeys={[active]}
