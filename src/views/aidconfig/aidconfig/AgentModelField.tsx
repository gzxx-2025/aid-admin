  ...
  return (
    <div style={{ width: '100%' }}>
      <Select
        ...
        notFoundContent={<span style={{ color: '#94a3b8' }}>暂无可选模型</span>}
      />
...
function capStr(m: any): string {
  try {
    const c = m.capabilities || m.capability || '';
    if (!c) return '';
    return typeof c === 'string' ? c.toLowerCase() : JSON.stringify(c).toLowerCase();
  } catch {
    return '';
  }
}
