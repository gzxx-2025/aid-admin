          ...
          >
            ...
            <Collapse
              ...
              items={EVENT_METAS.map((meta) => ({
                ...
                children: (
                  <div>
                    <div style={{ color: '#64748b', marginBottom: 12 }}>{meta.scene}</div>
                    <Form.Item
                      label="模板ID"
                      name={['templates', meta.key, 'templateId']}
                      extra="官方标准模板 ID，系统固定使用"
                    >
                      <Input disabled />
                    </Form.Item>
                    <Row gutter={[12, 0]}>
                      {meta.fields.map((field) => (
                        <Col span={12} key={field.name}>
                          <Form.Item
                            label={field.label}
                            name={['templates', meta.key, 'fields', field.name]}
                            extra={field.hint}
                          >
                            <Input disabled />
                          </Form.Item>
                ...
                children: (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {templateList.map((item) => (
                      <div key={item.templateId} style={{ borderBottom: '1px solid #f1f5f9', paddingBottom: 8 }}>
                        <Space wrap>
                          <strong>{item.title || '未命名模板'}</strong>
                          <Tag>{item.templateId}</Tag>
                        </Space>
                        {item.content && (
                          <div style={{ color: '#64748b', fontSize: 12, marginTop: 4, whiteSpace: 'pre-wrap' }}>
                            {item.content}
                          </div>
                        )}
                      </div>
