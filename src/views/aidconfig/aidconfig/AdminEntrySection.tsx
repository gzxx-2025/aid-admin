...
import { Alert, Button, Card, Input, InputNumber, Space, Switch, Tag, Tooltip, message } from 'antd';
import {
...
export default function AdminEntrySection() {
  ...
  const loginUrl = useMemo(() => {
    ...
    return enabled && code ? `${origin}/${code}` : origin + '/';
  }, [enabled, code]);
  ...
  return (
    <div style={{ maxWidth: 880 }}>
      <Card
        bordered={false}
        className="page-card"
        title={<Space><SafetyCertificateOutlined style={{ color: '#6366f1' }} />后台登录入口安全</Space>}
        extra={
          <Space>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
          <span style={{ width: 120, color: '#475569' }}>启用随机入口</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
          <span style={{ width: 120, color: '#475569' }}>访问码</span>
          <Input
            value={code}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
          <span style={{ width: 120, color: '#475569' }}>单IP每分钟尝试</span>
          <InputNumber
            value={rateLimit}
          ...
          <span style={{ color: '#94a3b8', fontSize: 12 }}>
            登录与访问码校验共用此阈值；填 0 表示不限流。默认 10。
          </span>
        </div>

        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ width: 120, color: '#475569' }}>登录地址</span>
          <Input
            readOnly
            value={loginUrl}
