    ...
    <div className="crud-page">
      ...
      <Card className="page-card" bordered={false}>
        <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
          ...
          <Auth permission="aid:voice-library:remove"><Button danger disabled={!selectedKeys.length} icon={<DeleteOutlined />} onClick={() => { delVoiceLibrary(selectedKeys.join(',')).then(() => { message.success('删除成功'); loadList(); }); }}>批量删除</Button></Auth>
        </div>
        <Table rowKey="id" size="small" loading={loading} dataSource={list} columns={columns} scroll={{ x: 1500 }}
