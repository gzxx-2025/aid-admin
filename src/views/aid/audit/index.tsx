...
import {
  Tabs, Table, Form, Input, Select, Button, Space, Tag, Modal, Descriptions, Empty, Spin, Image, message
} from 'antd';
...
export default function ComicAudit() {
  ...
  const renderSearch = (form: any, state: any, fields: SearchField[]) => (
    <Form form={form} layout="inline" onFinish={(v) => state.applySearch(v)} style={{ marginBottom: 12, rowGap: 8 }}>
      {fields.map((f) => (
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <Modal
        ...
        afterClose={() => { setDetail({}); setReason(''); }}
      ...
      >
        <Spin spinning={detailLoading}>
          <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
            <div style={{ flex: '0 0 430px', maxWidth: 430 }}>
              {/* 封面（项目/电影审核） */}
              {showCover && (
                <div style={{ marginBottom: showVideo ? 12 : 0 }}>
                  <div style={{ fontSize: 12, color: '#888', marginBottom: 4 }}>
                    {isMetadataReview ? '本次待审封面' : '封面'}
                    {isMetadataReview && <Tag color="processing" style={{ marginLeft: 4 }}>本次审核对象</Tag>}
                  </div>
                  {detail.coverUrl ? (
                    <Image src={detail.coverUrl} alt="封面" style={{ maxHeight: 240, borderRadius: 4 }} />
                  ) : (
                    <div style={{ height: 160, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fafafa', borderRadius: 4 }}>
                      <Empty description="暂无封面" imageStyle={{ height: 60 }} />
                    </div>
                  )}
                  {isMetadataReview && detail.publishedCoverUrl && (
                    <div style={{ marginTop: 12 }}>
                      <div style={{ fontSize: 12, color: '#888', marginBottom: 4 }}>当前线上封面</div>
                      <Image src={detail.publishedCoverUrl} alt="当前线上封面" style={{ maxHeight: 160, borderRadius: 4 }} />
                    </div>
                  )}
                </div>
              )}
              ...
              {showVideo && (
                <div>
                  ...
                  {detail.finalVideoUrl ? (
                    ...
                    <div style={{ height: 220, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fafafa', borderRadius: 4 }}>
              ...
              {detailKind === 'episode' && detail.comicCoverUrl && (
                <div style={{ marginTop: 12 }}>
                  ...
                  <Image src={detail.comicCoverUrl} alt="单集封面" style={{ maxHeight: 160, borderRadius: 4 }} />
                  {isMetadataReview && detail.publishedComicCoverUrl && (
                    <div style={{ marginTop: 12 }}>
                      <div style={{ fontSize: 12, color: '#888', marginBottom: 4 }}>当前线上单集封面</div>
                      <Image src={detail.publishedComicCoverUrl} alt="当前线上单集封面" style={{ maxHeight: 160, borderRadius: 4 }} />
                    </div>
                  )}
                </div>
              )}
