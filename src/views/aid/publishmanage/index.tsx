...
import {
  Tabs, Table, Form, Input, Select, Button, Space, Tag, Modal, Image, Switch, Tooltip, Avatar, Empty, message
} from 'antd';
...
function TypeTag({ type }: { type?: string }) {
  ...
  return <Tag color={type === 'movie' ? 'geekblue' : 'cyan'}>{type === 'movie' ? '电影' : '剧集'}</Tag>;
}
...
export default function PublishManage() {
  ...
  const openAction = (kind: keyof typeof actionConfig, row: PublishItem) => {
    ...
    Modal.confirm({
      ...
      content: (
        <div>
          <div style={{ color: '#888', fontSize: 12, marginBottom: 8 }}>{cfg.tip}</div>
          <Input.TextArea
            rows={3}
            maxLength={500}
            showCount
  ...
  const tabItems = [
    {
      ...
      children: (
        <>
          <Form form={publishForm} layout="inline" onFinish={(v) => publish.applySearch(v)} style={{ marginBottom: 12, rowGap: 8 }}>
    ...
    <div style={{ padding: 16 }}>
      ...
      >
        <div style={{ marginBottom: 12 }}>
          <div style={{ fontSize: 12, color: '#888', marginBottom: 4 }}>选择用户（昵称/邮箱/手机号搜索）</div>
          <Select
            showSearch
        ...
        <div>
          <div style={{ fontSize: 12, color: '#888', marginBottom: 4 }}>备注（选填）</div>
          <Input.TextArea
            rows={2}
            maxLength={100}
            showCount
