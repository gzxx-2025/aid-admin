...
export default function FuncconfigPage() {
  ...
  const columns: any[] = [
    ...
    { title: '功能编码', dataIndex: 'funcCode', width: 200, ellipsis: true,
      render: (v: string) => <code style={{ background: '#f1f5f9', color: '#475569', padding: '2px 8px', borderRadius: 6, fontSize: 12, fontFamily: 'Consolas, Menlo, monospace' }}>{v}</code> },
    ...
    <div className="crud-page">
      ...
      <Card className="page-card" bordered={false}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
          ...
          <div style={{ color: '#64748b', fontSize: 13 }}>共 <b style={{ color: '#1f2937' }}>{total}</b> 条</div>
        </div>
      ...
      <Modal open={dlgOpen} title={<Space><AppstoreOutlined style={{ color: '#2563eb' }} /><span>{dlgTitle}</span></Space>} onCancel={() => setDlgOpen(false)} onOk={handleSave} confirmLoading={saving} width={1180} destroyOnClose maskClosable={false}>
        <Form form={form} layout="vertical" style={{ marginTop: 8 }}>
          ...
          <div style={{ marginBottom: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontWeight: 600, fontSize: 14, color: '#1f2937' }}>可选模型</span>
            <span style={{ color: '#94a3b8', fontSize: 12 }}>必选；可按「输入要求」区分图片必传 / 图片可选等类型</span>
          </div>
