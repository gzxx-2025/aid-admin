...
import { Badge, Button, Input, Select, Space, Switch, Table, Tag, Tooltip, message } from 'antd';
...
export default function ModelTable({ provider, list, loading, query, onQueryChange, onAdd, onEdit, onDelete, onToggleStatus, onSyncVoice, onCleanExpired, onOpenOverview }: Props) {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#fff', padding: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
        ...
        <Space size={8} wrap>
          ...
          {provider.providerCode?.trim().toLowerCase() === 'minimax' && onSyncVoice && (
            <Badge count="推荐" size="small" style={{ backgroundColor: '#f5222d' }}>
              <Button type="default" size="small" icon={<SyncOutlined />} onClick={onSyncVoice} style={{ color: '#52c41a', borderColor: '#52c41a' }}>
                同步音色
              </Button>
            </Badge>
          )}
      ...
      <Table rowKey="id" size="small" loading={loading} dataSource={list} columns={columns} scroll={{ x: 1960 }} pagination={false} />
