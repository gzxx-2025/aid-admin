...
import { Button, Input, InputNumber, Modal, Space, Switch, Form, message } from 'antd';
    ...
    <div>
      ...
      {isTokenBilling && (
        <div style={{ marginBottom: 12 }}>
          ...
          <span style={{ fontSize: 12, color: '#94a3b8', marginLeft: 8 }}>计算时：字符数 ÷ 此比例 ≈ Token 数</span>
        </div>
      )}
          ...
          <div>
            {isTokenBilling ? (
              ...
              <Space wrap align="end">
                ...
                <Field label="整包官方原价（元，选填；每秒原价缺省时用整包价÷时长上限反推）" width={360}>
              ...
              <Space wrap align="end">
                ...
                <Field label="兜底单次官方原价（元，选填；每字符原价未配时使用）" width={350}>
                  <InputNumber size="small" style={{ width: 130 }} min={0} precision={6} value={sku.price} onChange={(v) => updateSku(idx, { price: v })} />
                </Field>
              </Space>
            ) : (
              <Space wrap align="end">
                <Field
                  label={mt === 'PER_IMAGE' ? '每张官方原价（元/张）'
                    : mt === 'SKU_PACKAGE' ? '整包官方原价（元/次）'
                    : '固定官方原价（元/次）'}
                  width={260}
                >
                  <InputNumber size="small" style={{ width: 130 }} min={0} precision={6} value={sku.price} onChange={(v) => updateSku(idx, { price: v })} />
      ...
      {/* 试算 */}
      {data.skuList.length > 0 && (
        <TrialCalc isTokenBilling={isTokenBilling} loading={previewLoading} result={previewResult} onCalc={onPreview} />
      )}
        ...
        <Form layout="vertical">
          ...
          <div style={{ fontSize: 12, color: '#94a3b8', lineHeight: '20px' }}>
            ...
            {Object.entries(MATCH_KEY_HINTS).map(([k, hint]) => (
              <div key={k}>
                <code style={{ color: '#667eea' }}>{k}</code>：{hint}
              </div>
            ))}
          </div>
        </Form>
...
function TrialCalc({ isTokenBilling, loading, result, onCalc }: { isTokenBilling: boolean; loading: boolean; result: PreviewResult | null; onCalc: (i: number, o: number) => void }) {
  ...
  return (
    <div style={{ marginTop: 12, padding: 12, background: 'linear-gradient(90deg, rgba(102,126,234,0.06), rgba(118,75,162,0.06))', borderRadius: 8, border: '1px dashed rgba(102,126,234,0.3)' }}>
      <div style={{ fontWeight: 500, fontSize: 13, marginBottom: 8, color: '#667eea' }}>💡 试算测试</div>
