...
export default function UserManagePage() {
  ...
  const logColumns = [
    ...
    },
    {
      ...
      width: 110,
      render: (v: number) => (
        <span style={{ color: Number(v) >= 0 ? '#16a34a' : '#dc2626', fontWeight: 600 }}>
          {Number(v) >= 0 ? '+' : ''}
          {fmtMoney(v)}
        </span>
      )
      ...
      >
        {current && (
          <>
            ...
            {/* 账户统计 */}
            <Row gutter={12} className="detail-stats">
              <Col xs={12} sm={6}>
                <Card size="small" bordered={false} className="stat-card stat-card--primary">
                  <Statistic title="账户余额" prefix="¥" value={fmtMoney(current.balance)} />
                </Card>
              </Col>
              <Col xs={12} sm={6}>
                <Card size="small" bordered={false} className="stat-card">
                  <Statistic title="冻结余额" prefix="¥" value={fmtMoney(current.frozenBalance)} />
                </Card>
              </Col>
              <Col xs={12} sm={6}>
                <Card size="small" bordered={false} className="stat-card">
                  <Statistic title="累计充值" prefix="¥" value={fmtMoney(current.totalRecharge)} />
                </Card>
              </Col>
              <Col xs={12} sm={6}>
                <Card size="small" bordered={false} className="stat-card">
                  <Statistic title="累计消费" prefix="¥" value={fmtMoney(current.totalConsumption)} />
                </Card>
              </Col>
            </Row>
