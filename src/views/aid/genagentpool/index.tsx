    ...
    <div style={{ padding: 16 }}>
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        <Text type="secondary" style={{ marginLeft: 12 }}>
          {isFixed
            ? '说明：资产/分镜生图场景不随创作模式变；每个业务场景配经济/性能默认智能体与模型' + (isImage ? '（含清晰度/比例）' : '') + '，可加额外候选进可选池。'
            : '说明：每个「创作模式 × 剧本类型」是一个组合；经济/性能各选默认智能体与模型，可再加额外候选进可选池。'}
        </Text>
      </div>
      <Table
      ...
      <Modal
        ...
        destroyOnClose
      >
        <Form form={form} labelCol={{ span: 6 }} wrapperCol={{ span: 16 }} style={{ marginTop: 8 }}>
          {isFixed ? (
