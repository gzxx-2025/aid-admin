...
import { AutoComplete, Col, DatePicker, Form, Image, Input, InputNumber, Row, Select, Switch } from 'antd';
...
/** 图片字段：URL 输入 + 实时缩略预览（受控，供 Form.Item 注入 value/onChange） */
function ImageUrlField({ value, onChange, placeholder, disabled }: {
  ...
  return (
    <div>
      ...
      />
      {value ? (
        <Image src={value} width={96} height={96} style={{ marginTop: 8, borderRadius: 6, objectFit: 'cover' }} />
      ) : null}
    </div>
  );
}
...
export default function FormRenderer({ fields, form, isEdit }: Props) {
  ...
  const renderField = (field: FieldConfig) => {
    ...
    switch (field.type) {
      ...
      case 'image':
        return <ImageUrlField placeholder={placeholder} disabled={field.disabled} />;
      case 'combobox':
